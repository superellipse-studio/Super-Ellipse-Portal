...
export default function TasksClient({ tasks, projects, timeline, team }: { tasks: Task[]; projects: Project[]; timeline: TimelineItem[]; team: TeamMember[] }) {
  ...
  return <>
    ...
    {show&&<div className="fixed inset-0 z-50 bg-black/55 backdrop-blur-md flex items-center justify-center p-4"><form onSubmit={submit} className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-3xl bg-sage text-black p-6 space-y-5 shadow-2xl ring-1 ring-white/20"><div className="flex justify-between"><h2 className="text-2xl font-bold">New Task</h2><button type="button" onClick={()=>setShow(false)}>✕</button></div>
      ...
      {form.project_id&&form.project_id!=="studio"&&<label className="block"><span className="label text-black/50">Project Stage</span><select value={form.timeline_id} onChange={e=>{const selected=stages.find(s=>s.id===e.target.value);setForm({...form,timeline_id:e.target.value,due_date:selected?.end_date||form.due_date})}} className="mt-2 w-full rounded-xl border border-black/20 bg-white/45 p-3 outline-none focus:border-black/45"><option value="">No stage</option>{stages.map(s=><option key={s.id} value={s.id}>{s.label} ({s.start_date} → {s.end_date})</option>)}</select></label>}
