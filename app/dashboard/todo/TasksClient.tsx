...
function TaskRow({ task }: { task: Task }) {
  return <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1.5 py-3">
    ...
    <div className="flex gap-3 pl-7 sm:pl-0 text-sm text-black/45">{(task.start_date || task.due_date) && <span>{task.start_date && task.due_date ? `${task.start_date} → ${task.due_date}` : task.due_date ? `End ${task.due_date}` : `Start ${task.start_date}`}</span>}<span className={cardLabel}>{task.assignee}</span></div>
  </div>;
}
...
export default function TasksClient({ tasks, projects, timeline, team }: { tasks: Task[]; projects: Project[]; timeline: TimelineItem[]; team: TeamMember[] }) {
  ...
  const [form,setForm]=useState({title:"",project_id:"",timeline_id:"",stage_label:"",start_date:"",due_date:"",assignee:"You"});
  ...
  async function submit(e:React.FormEvent){e.preventDefault();setSaving(true);setError(""); const {stage_label: _stageLabel, ...formPayload}=form; const payload={...formPayload,scope:"this_week"}; const r=await fetch('/api/tasks/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); const j=await r.json(); setSaving(false); if(!r.ok){setError(j.error||'Could not create task');return;} setShow(false);setForm({title:"",project_id:"",timeline_id:"",stage_label:"",start_date:"",due_date:"",assignee:"You"});router.refresh();}
  return <>
    ...
    {show&&<div className="fixed inset-0 z-50 bg-black/55 backdrop-blur-md flex items-center justify-center p-4"><form onSubmit={submit} className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-3xl bg-sage text-black p-6 space-y-5 shadow-2xl ring-1 ring-white/20"><div className="flex justify-between"><h2 className="text-2xl font-bold">New Task</h2><button type="button" onClick={()=>setShow(false)}>✕</button></div>
      <label className="block"><span className="label text-black/50">Description</span><textarea required value={form.title} onChange={e=>setForm({...form,title:e.target.value})} className="mt-2 w-full rounded-xl border border-black/20 bg-white/45 p-3 outline-none focus:border-black/45" rows={3}/></label>
      <label className="block"><span className="label text-black/50">Project</span><select required value={form.project_id} onChange={e=>{const project=projects.find(p=>p.id===e.target.value);setForm({...form,project_id:e.target.value,timeline_id:"",stage_label:project?.current_phase||""})}} className="mt-2 w-full rounded-xl border border-black/20 bg-white/45 p-3 outline-none focus:border-black/45"><option value="">Choose project</option>{ongoing.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}<option value="studio">Studio / Internal</option></select></label>
      {form.project_id&&form.project_id!=="studio"&&<label className="block"><span className="label text-black/50">Project Stage</span><select value={form.stage_label} onChange={e=>{const label=e.target.value;const selected=stages.find(s=>s.label.toLowerCase()===label.toLowerCase());setForm({...form,stage_label:label,timeline_id:selected?.id||"",start_date:selected?.start_date||form.start_date,due_date:selected?.end_date||form.due_date})}} className="mt-2 w-full rounded-xl border border-black/20 bg-white/45 p-3 outline-none focus:border-black/45"><option value="">No stage</option>{PROJECT_PHASES.map(label=><option key={label} value={label}>{label}{selectedProject?.current_phase?.toLowerCase()===label.toLowerCase()?" — Current":""}</option>)}</select><p className="mt-2 text-xs text-black/45">Synced with the same project phases used on the Projects page. The current phase is selected automatically.</p></label>}
      <div className="grid sm:grid-cols-2 gap-4"><label><span className="label text-black/50">Start Date</span><input required type="date" value={form.start_date} onChange={e=>setForm({...form,start_date:e.target.value})} className="mt-2 w-full rounded-xl border border-black/20 bg-white/45 p-3 outline-none focus:border-black/45"/></label><label><span className="label text-black/50">End Date</span><input required type="date" min={form.start_date || undefined} value={form.due_date} onChange={e=>setForm({...form,due_date:e.target.value})} className="mt-2 w-full rounded-xl border border-black/20 bg-white/45 p-3 outline-none focus:border-black/45"/></label><label><span className="label text-black/50">Assignee</span><select value={form.assignee} onChange={e=>setForm({...form,assignee:e.target.value})} className="mt-2 w-full rounded-xl border border-black/20 bg-white/45 p-3 outline-none focus:border-black/45"><option>You</option>{team.map(m=><option key={m.id}>{m.name}</option>)}</select></label></div>
